...
## &hellip; en de formele NIST-definitie {.smaller}
...
## CIA gaat dieper: elk met twee gezichten {.smaller}
...
## McCumber in actie: Twitter's hash-bug (2018) {.smaller}
...
::: {.column width="55%"}
![](assets/mccumber_twitter.png){ width=100% fig-alt="De McCumber kubus met de schijf verwerken rood gearceerd: daar lekten de wachtwoorden, terwijl de opslag in orde was." }
:::
...
![](assets/scheve_grafiek.png){ width=62% fig-align="center" fig-alt="Grafiek: de kracht van aanvalstools stijgt van 1980 tot 2020, terwijl de vereiste kennis van de aanvaller daalt." }
...
## Zero days en patching {.smaller}

* **Zero day** = kwetsbaarheid die nog onbekend is bij de softwarefabrikant
  * Gegarandeerd werkzaam - er bestaat nog geen patch

![](assets/window_vuln.png){.nostretch width=80% fig-align="center" fig-alt="Tijdlijn: tussen het ontdekken van een zero day en de patch ligt het window of vulnerability. Daarna blijven niet-gepatchte systemen kwetsbaar." }
...
## Wie zijn de stropers? {.ai-beeld}

![](assets/stropers.png){fig-alt="Een boswachter bekijkt verse voetafdrukken, terwijl tussen de bomen heel verschillende figuren toekijken."}
...
## Werknemers: de vergeten bedreiging {.smaller}
...
## Net ontslagen&hellip; {.ai-beeld}

![](assets/akhter_meeting.png){fig-alt="Een laptop op de keukentafel: de online meeting is net afgelopen, de handen hangen boven het toetsenbord."}

---

## `DROP DATABASE;` - de gebroeders Akhter (2025) {.smaller}
...
* **13 jaar cel**: maar het verhaal stopt daar niet&hellip;
...
## Cyberterroristen, spionnen & overheden {.smaller}
...
::: {.fragment}
![Stuxnet: door staten gesponsorde malware die Iraanse centrifuges saboteerde.](assets/stuxnet.png){ width=35% }
:::

---

## Een (onvolledige) geschiedenis {.smaller}
...
## Hacktivisme in de Oekraïne-oorlog {.smaller}
...
![](assets/dwelltime.png){ width=100% fig-alt="Tijdlijn: 75% van de aanvallers is binnen in minuten, 54% wordt pas na maanden ontdekt." }
...
## Hoe verdedigen? De 4+1 principes {.smaller}
...
::: {.column width="42%"}
![](assets/ui_layering.png){ width=100% fig-alt="Verschillende lagen als een ui rond data, gebruikers en services: de aanvaller kraakt de buitenste laag, de volgende houdt stand." }
:::
...
*Hoe zet een aanvaller een eerste voet binnen?* - *initial access vectors*:

::: {.incremental}
* **Social engineering** &rarr; phishing, vishing, smishing, &hellip; *(deze sectie)*
* **Zero-day exploit** &rarr; eerder bij *Zero days en patching*
* **Insider threat** &rarr; eerder bij *Werknemers*
* **Supply chain attack** &rarr; hoofdstuk 1 (SolarWinds, NotPetya, CCleaner, Hezbollah-pagers&hellip;)
* **Drive-by download & watering hole** &rarr; straks bij *Software-based aanvallen*
:::

::: {.callout-tip .fragment}
Vraag jezelf bij elke aanval die je analyseert eerst af: *via welke vector* is de stroper binnen geraakt?
:::
...
## OSINT: Open Source Intelligence {.smaller}
...
![Het worm propagation model.](assets/wormprop.png){ width=65% }
...
## Hoe lang overleeft deze pc? {.ai-beeld}

![](assets/xp_wekker.png){fig-alt="Een gloednieuwe pc wordt aan het netwerk gehangen, met een keukenwekker ernaast."}

---

## Survival time: 20 minuten

::: {.incremental}
* **Blaster** (2003) en **Sasser** (2004): *remote execution* via gaten in Windows (RPC/DCOM, LSASS)
* **Geen klik nodig**: een ongepatchte XP-machine met publiek IP aan het netwerk hangen volstond
* SANS Internet Storm Center, 2004: gemiddeld **± 20 minuten** tot besmetting
* Alle patches downloaden duurde langer &rarr; je verloor de race tegen Windows Update
:::
...
## ILOVEYOU vs. SQL Slammer {.smaller}
...
## Drive-by download & watering hole {.smaller}

::: {.incremental}
* **Drive-by download**: een gecompromitteerde webpagina bezoeken volstaat - *geen klik nodig*. Misbruik van browser-, plugin- of PDF-kwetsbaarheid
* **Watering hole**: aanvaller infecteert een site die zijn doelwit dagelijks raadpleegt (bv. vak-nieuwssite van een defensieleverancier) - *gerichte* variant
* **2013**: bezoekers van een denktank-site &rarr; Department of Labor, NIST en defensiecontractors besmet *zonder één klik op een e-mail*
:::

::: {.callout-tip .fragment}
Nauw verwant: **malvertising** - kwaadaardige advertentiebanners via reguliere ad-netwerken op anders gerespecteerde websites.

**Verdediging**: patch je browser én gebruik een ad-blocker.
:::
...
## Supply-chain trojan: CIA &amp; Xcode {.smaller}
...
## Wat hebben deze toestellen gemeen? {.ai-beeld}

![](assets/mirai_woonkamer.png){fig-alt="Een gezellige woonkamer met een babyfoon-camera, een router en een slimme camera. Op de babyfoon brandt een klein rood lampje."}
...
## *"DDoS à la carte"* - echte afpersingsbrief {.smaller}
...
## Ergens in Kaboel, 2008 {.ai-beeld}

![](assets/kabul_usb.png){fig-alt="Een hand in legeruniform pakt een goedkope USB-stick van een marktkraam, met een militaire compound op de achtergrond."}

---

## De Russische USB's van Kabul (2008) {.smaller}
...
## Clickjacking op Android {.smaller}
...
## Je telefoon staat uit. Toch? {.ai-beeld}

![](assets/poweroff_nachtkastje.png){fig-alt="Een smartphone ligt omgekeerd op een nachtkastje, maar aan de rand brandt nog een piepklein rood lampje."}

---

## Android/PowerOffHijack (2015) {.smaller}
...
## Whisper Leak (nov 2025): LLM-gesprekken lekken {.smaller}
