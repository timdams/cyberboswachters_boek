...
![Bij 6 gebruikers zijn er al 15 sleutels nodig.](crypto_assets/keyprob.png){ width=55% }
...
![](crypto_assets/kist_metafoor.png){.nostretch width=82% fig-align="center" fig-alt="Een openstaande kist die iedereen kan dichtklikken, een kist op slot, en de eigenaar die de kist weer opent." }
...
![Het ondertekenen van een document met behulp van je private sleutel.](crypto_assets/handtekening.png)
...
1. Alice en Bob spreken **publiek** twee parameters af: een priemgetal $p$ en een basis $g$
2. Elk kiest daarnaast een **geheime waarde** (enkel voor deze sessie)
3. Ze berekenen elk een **publieke waarde** $g^{geheim}\%p$ en sturen die naar de ander (mag onbeveiligd)
4. Elk combineert de ontvangen publieke waarde met de eigen geheime waarde
5. Resultaat: een **shared secret** dat beiden kennen
...
## Diffie-Hellman: de verfkleurenmetafoor {.smaller}
...
::: {.column width="45%"}
![](crypto_assets/dh-paint.png){width=55%}
:::
...
## Diffie-Hellman uitgelegd in filmpje {.smaller}

![Alles samen uitgelegd](crypto_assets/ssh.mp4)
...
![](crypto_assets/hash_avalanche.png){width=85% fig-alt="Twee zinnen die maar één teken verschillen geven via SHA-256 een totaal andere hash."}
...
![](crypto_assets/cert_verificatie.png){fig-alt="Links ondertekent de CA het certificaat, rechts vergelijkt de browser de zelf berekende hash met de ontsleutelde handtekening."}
...
![](crypto_assets/chain_of_trust.png){.nostretch width=62% fig-align="center" fig-alt="Keten van CA's onder een root CA: wordt de private sleutel van een CA gestolen, dan worden alle certificaten eronder ongeldig."}



---

## Wie vertrouw je nog? {.ai-beeld}

![](assets/crypto_diginotar.png){fig-alt="In een notariskantoor drukt iemand ongemerkt een nagemaakt rood zegel op een stapel certificaten."}
...
::: {.column width="60%"}
![](crypto_assets/weboftrust.png){width=100% fig-alt="Carol vertrouwt Alice, Alice ondertekent Bobs publieke sleutel, en zo aanvaardt Carol via een vertrouwenspad ook Bobs sleutel."}
:::
...
![](crypto_assets/mtls_simpel.png){.nostretch width=65% fig-align="center" fig-alt="Bij mTLS toont niet enkel de server maar ook de client een certificaat, beide gecontroleerd bij dezelfde CA."}
...
## Een extra deurtje, enkel voor de goeden? {.ai-beeld}

![](assets/crypto_backdoor.png){fig-alt="Achteraan een goed beveiligd huis bouwen ambtenaren een klein rood deurtje in, terwijl een inbreker vanuit de struiken toekijkt."}
